...
def test_basis_adjustment_homogeneous_hazards_matches_index() -> None:
    ...
    adjusted, betas, adjusted_index = basis_adjust_curves_beta(curves, index_curve, recovery=0.4)
    ...
    for t in times:
        ...
        assert np.isclose(
            adjusted_index.default_prob(float(t)),
            index_curve.default_prob(float(t)),
            rtol=1e-6,
            atol=1e-8,
        )
...
def test_basis_adjustment_matches_index_expected_loss() -> None:
    ...
    adjusted, _, adjusted_index = basis_adjust_curves_beta(curves, index_curve, recovery=recovery)
    ...
    for t in times:
        ...
        adj_index_el = (1.0 - recovery) * adjusted_index.default_prob(float(t))
        assert np.isclose(adj_index_el, target_el, rtol=1e-4, atol=5e-5)
